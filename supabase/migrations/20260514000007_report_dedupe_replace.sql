-- Flax HR — Reports fix (retry): CREATE OR REPLACE instead of DROP
--
-- The previous attempt (20260514000006) used DROP VIEW IF EXISTS on
-- v_daily_attendance and v_attendance_report. That failed at prod apply
-- with:
--   2BP01: cannot drop view v_attendance_report because other objects
--   depend on it — v_employee_snapshot depends on it.
--
-- Column lists in the new definitions are unchanged from the originals,
-- so CREATE OR REPLACE VIEW is safe and side-steps the dependency check.
-- v_employee_snapshot keeps working through the swap.

----------------------------------------------------------------------------
-- v_daily_attendance: one row per (employee, work_date), cross-outlet
----------------------------------------------------------------------------
